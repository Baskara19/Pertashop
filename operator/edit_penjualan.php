...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    $nama=$_POST['nama']; $shift=$_POST['shift'];
    $odo_awal=(int)$_POST['odo_awal']; $odo_akhir=(int)$_POST['odo_akhir'];
    $ukur_awal=(float)$_POST['ukur_awal']; $ukur_akhir=(float)$_POST['ukur_akhir'];
    ...
    header("Location: laporan_operator.php?tanggal=".($_POST['tanggal'] ?? date('Y-m-d'))); exit;
}
$stmt=$pdo->prepare("SELECT * FROM penjualan_harian WHERE id=?"); $stmt->execute([$id]);
$data=$stmt->fetch(); if (!$data) { die("Data tidak ditemukan!"); }
?>
...
<div class="form-box"><h2>Edit Data Penjualan</h2>
<form method="POST">
...
<a href="laporan_operator.php?tanggal=<?= $data['tanggal'] ?>">⬅️ Kembali ke Laporan</a>
</div></body></html>
