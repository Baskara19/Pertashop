    ...
    if ($row) {
        if (password_verify($password, $row['password'])) {
            if ($row['role'] !== $role) {
                ...
                if ($row['role'] === 'admin') { session_name("PERTASHOP_ADMIN"); }
                elseif ($row['role'] === 'operator') { session_name("PERTASHOP_OPERATOR"); }
                session_start();
                ...
                else { header("Location: operator/dashboard_operator.php"); }
                exit();
            }
...
<head>
    ...
    <style>
        ...
        button{width:100%;padding:12px;background:#5a0000;color:white;border:none;border-radius:6px;font-size:16px;cursor:pointer}
        button:hover{background:#cb3a00}.error{color:red;margin-top:10px;font-size:14px}
    </style>
</head>
<body>
<div class="login-box">
