...
if ($_SERVER["REQUEST_METHOD"]==="POST") {
    ...
    elseif ($harga_pertamax<=0) { $error="Harga pertamax harus lebih dari 0."; }
        ...
        $penjualan_liter=$odo_akhir-$odo_awal; $penghasilan=$penjualan_liter*$harga_pertamax;
        $hasil_pengukuran=($pengukuran_awal-$pengukuran_akhir)*21.23; $stok_pertamax=$pengukuran_akhir*21.23;
        $stmt=$pdo->prepare("INSERT INTO penjualan_harian (nama,tanggal,shift,odo_awal,odo_akhir,penjualan_liter,penghasilan_rp,ukur_awal,ukur_akhir,harga_pertamax,stok_hari_ini) VALUES (?,?,?,?,?,?,?,?,?,?,?)");
        if ($stmt->execute([$nama,$tanggal,$shift,$odo_awal,$odo_akhir,$penjualan_liter,$penghasilan,$pengukuran_awal,$pengukuran_akhir,$harga_pertamax,$stok_pertamax])) {
...
<meta charset="UTF-8"><title>Input Penjualan Pertashop</title><meta name="viewport" content="width=device-width,initial-scale=1.0">
<style>
body{font-family:'Segoe UI',Tahoma,sans-serif;background:linear-gradient(135deg,#e6ecf3,#f8fbfd);display:flex;justify-content:center;align-items:flex-start;min-height:100vh;margin:0;padding:20px}
...
.btn-primary{padding:11px 18px;border-radius:10px;font-weight:600;border:none;cursor:pointer;background:linear-gradient(45deg,#007bff,#00c6ff);color:white;font-size:14px}
.btn-secondary{padding:11px 18px;border-radius:10px;font-weight:600;background:linear-gradient(45deg,#28a745,#6edc82);color:white;text-decoration:none;display:inline-block;font-size:14px}
@media(max-width:768px){.grid-2{grid-template-columns:1fr}}
</style></head><body>
<div class="form-container">
<div class="top-bar"><a class="btn-secondary" href="dashboard.php">⬅️ Kembali ke Dashboard</a></div>
